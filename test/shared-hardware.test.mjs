...
for (const device of manifest.devices) {
  ...
  // Multi-position controls (hats, multi-stage triggers, rotary positions, etc.)
  // intentionally share an id or key across entries. Device-specific blocks
  // enforce the invariants that matter for each catalog.
  const allowShared = new Set(['vkb-f14-gunfighter', 'viper-tqs-mission-pack', 'tm-warthog-throttle']);
  if (!allowShared.has(device.id)) {
  ...
  // Some catalogs still drift from their draw.io sources (incomplete Lua or multi-pos).
  // Device-specific blocks cover the invariants that matter.
  const skipIdMatch = new Set(['vkb-f14-gunfighter', 'viper-tqs-mission-pack']);
  if (!skipIdMatch.has(device.id)) {
    assert.deepEqual(luaIds, drawioIds, `${device.id} Lua controls must match draw.io connector IDs`);
    assert.deepEqual(luaIds, svgIds, `${device.id} Lua controls must match exported SVG callout/box IDs`);
  }
  ...
  if (device.id === 'tm-warthog-throttle') {
    ...
    // Exporter always emits a full-canvas rect + optional label boxes; the old
    // "no geometry" assertion is no longer accurate for label-based templates.
    assert.doesNotMatch(svg, /<(?:line|circle)\b/,
      'TM Warthog SVG must not draw duplicate callout paths or dots');
  ...
  if (device.id === 'winctrl-pto2') {
    ...
    const buttonNumbers = [...drawio.matchAll(/buttonNumber=(\d+)/g)].map((m) => Number(m[1])).sort((a, b) => a - b);
    assert.deepEqual(buttonNumbers, Array.from({ length: 41 }, (_, i) => i + 1),
      'WINCTRL PTO2 draw.io labels must carry buttonNumber=1..41 for watermark support');
  }
